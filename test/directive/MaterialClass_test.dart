...
main() async {
    ...
    group('MaterialClass', () {
        ...
        test('> Upgrade', () async {
            ...
            final mc = MaterialClass.widget(div);
            expect(mc, isNotNull);
        }); // end of 'Upgrade' test

        test('> Change class via model', () async {
            final dom.DivElement div = await renderer.render(parent,html) as dom.DivElement;
            await componentHandler().upgradeElement(div);
            ...
            expect(div.classes.toList().length,4);

            final testStore = ioc.IOCContainer().resolve(service.SimpleDataStore).as<_TestStore>();
            testStore.withBorder = true;

            await waitUntil(() => div.classes.contains("withborder"));

            expect(div.classes.contains("withborder"), isTrue);
            expect(div.classes.toList().length,5);
