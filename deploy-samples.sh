...
usage() {
    ...
    echo -e "\t-d | --deploy   [example_name] [--release|dev]   Creates 'deploy'-dir for Dart"
    echo -e "\t-p | --publish  [example_name] [--force]         Publish samples to AWS/S3"
...
case "${CMDLINE}" in
    ...
    -d|deploy|-deploy|--deploy)
        if [ -n "${OPTION1+set}" -a "${OPTION1}" != "--release" -a "${OPTION1}" != "--dev" -a "${OPTION1}" != "" ]; then
